...
def gen_spec(spec_file, config_file):
  ...
  # Apply extension properties to the spec according to the overrides in the config file.
  apply_extn_spec(config)
...
def apply_extn_spec(config):
  """Apply openapi extension property to endpoints as 'x-public:true' to the overrides in the config file."""
  # Endpoints to be included
  for ep in config.inclusions:
    try:
      public = config.spec['paths'][ep.path][ep.method]
      public.append('x-public: true')
    except KeyError:
...
def filter_spec(spec):
  ...
  for path in spec['paths']:
    for method in spec['paths'][path]:
      ...
      public = ep.get("x-public")
      selfhosted = env.get('self-hosted')
      if ((public is True) and (selfhosted is True)):
